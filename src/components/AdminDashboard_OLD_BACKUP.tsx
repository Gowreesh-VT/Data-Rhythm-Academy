...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onNavigate, onLogout }) => {
  ...
  const handleCreateCourse = async (courseData: any) => {
    console.log('handleCreateCourse: Starting course creation with data:', courseData);
    
    if (!user?.id) {
      console.error('handleCreateCourse: User not authenticated');
      showError('Authentication Error', 'You must be logged in as an admin');
      return;
    }
    ...
    if (!courseData.title || !courseData.instructorId || !courseData.category) {
      console.error('handleCreateCourse: Missing required fields', {
        hasTitle: !!courseData.title,
        hasInstructorId: !!courseData.instructorId,
    ...
    try {
      ...
      console.log('handleCreateCourse: Found instructor:', instructor);
      ...
      const newCourseData: Omit<Course, 'id' | 'createdAt' | 'updatedAt'> = {
        ...
        instructorName: courseData.instructorName || instructor?.displayName || 'Unknown Instructor',
        ...
        prerequisites: Array.isArray(courseData.prerequisites) 
          ...
          : (courseData.prerequisites || '').split(',').map((s: string) => s.trim()).filter((s: string) => s),
        tags: Array.isArray(courseData.tags) 
          ? courseData.tags 
          : (courseData.tags || '').split(',').map((s: string) => s.trim()).filter((s: string) => s),
      ...
      console.log('handleCreateCourse: Calling createCourse with:', newCourseData);
      const result = await createCourse(newCourseData, user.id);
      
      if (result.error) {
        console.error('handleCreateCourse: createCourse returned error:', result.error);
        showError('Creation Failed', result.error.message);
      } else {
        console.log('handleCreateCourse: Course created successfully:', result.data);
        success('Success', `Course "${courseData.title}" created successfully`);
        setShowCreateCourseModal(false);
        loadCoursesData(); // Refresh courses
      }
    } catch (error: any) {
      console.error('handleCreateCourse: Exception caught:', error);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <Dialog 
        open={showUserModal} 
        onOpenChange={(open) => {
          console.log('User Modal onOpenChange called with:', open);
          console.log('Current showUserModal state:', showUserModal);
          
          // Prevent auto-closing on mount
          if (!open && showUserModal) {
            console.log('Preventing user modal from closing immediately after opening');
            return;
          }
...
const CourseCreationForm: React.FC<CourseCreationFormProps> = ({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log('CourseCreationForm: Form submitted with data:', formData);
    ...
    if (!formData.level) {
      console.error('Validation failed: No level selected');
      alert('Please select a difficulty level');
      return;
    }
    if (!formData.shortDescription.trim()) {
      console.error('Validation failed: Short description is empty');
    ...
    console.log('CourseCreationForm: Submitting course data:', courseData);
    onSubmit(courseData);
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      {availableInstructors.length === 0 && (
        <Alert className="border-yellow-200 bg-yellow-50">
          ...
          <AlertDescription className="text-yellow-800">
            No instructors available. Please create an instructor account first or assign the instructor role to an existing user.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          >
            <SelectTrigger>
              <SelectValue placeholder={availableInstructors.length === 0 ? "No instructors available" : "Select instructor"} />
            </SelectTrigger>
            <SelectContent>
      ...
      <div className="flex justify-end space-x-2 pt-4">
        ...
        <Button 
          type="submit" 
          disabled={isCreating || availableInstructors.length === 0}
        >
